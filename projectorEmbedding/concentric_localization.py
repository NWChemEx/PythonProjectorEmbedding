...
class ConcentricLocalizer():
    ...
    def __init__(
        ...
        self._n_active_atoms = len(active_atoms)
    ...
    def localize_virtual(self) -> scf.hf.SCF:
        ...
        """
        print("Localising virtual orbital spin with concentric localization.")

        print("Creating projected molecule object.")
        embedded_scf = self.embedded_scf
        print(f"{embedded_scf.mol.atom=}")
        print(f"{embedded_scf.mol.charge=}")
        print(f"{embedded_scf.mol.spin=}")
        ...
        print(f"{self.active_atoms=}")
        n_act_proj_aos = projected_mol.aoslice_by_atom()[self.active_atoms[-1]][-1]
        print(f"{n_act_proj_aos=}")
        ...
        print("Completed Concentric Localization.")
        print(f"{self.shells=}")
        print(f"{self.singular_values=}")
        return embedded_scf
    ...
    def _localize_virtual_spin(
        ...
        """
        print("Running concentric localiztion for single spin.")
        print(f"{occ=}")
        print(f"{mo_coeff.shape=}")
        print(f"{fock_operator.shape=}")

        effective_virt = mo_coeff[:, occ == 0]
        print(f"N effective virtuals: {effective_virt.shape}")
        ...
        print(f"Initial {c_total.shape=} (nocc)")
        print(f"{self.n_act_proj_aos=}")
        ...
        print("Created 0th shell.")

        if v_ker.shape[-1] == 0:
            print("No kernel for 0th shell, cannot perform CL.")
            print(
            ...
            for ishell in range(0, self.max_shells):
                ...
                print(f"{shell_size=}")
                if shell_size == 0:
                    print("Empty shell %s, ending CL.", ishell)
                    c_total = np.concatenate((c_total, c_iker), axis=-1)
                    break
                ...
                if v_ker.shape[-1] > 1:
                    print("Kernel dimension is greater than 1, continuing CL.")
                    # in-place update
                    c_iker = c_iker @ v_ker
                elif v_ker.shape[-1] == 1:
                    c_iker = c_iker @ v_ker
                ...
                if ishell >= self.max_shells:
                    print("Max shells reached, not localizing further virtuals.")
                    c_total = np.concatenate((c_total, c_iker), axis=-1)
                    shells.append(c_total.shape[-1])
                    break

        print(f"Shell indices: {shells}")

        print(f"{c_total, shells, singular_values}")
